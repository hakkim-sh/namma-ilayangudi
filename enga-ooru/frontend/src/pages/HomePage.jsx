  ...
  const saveEdit = async (updatedData) => {
    try {
      const updatedListing = await updateListing(
        editingListing.listing._id || editingListing.listing.id,
        updatedData,
        editingListing.managementKey
      )
      setEditingListing(null)
      if (selectedListing?._id === updatedListing._id) {
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <form onSubmit={submitSearch} className="mt-8 flex max-w-3xl items-center gap-3 rounded-full border border-blue-100 bg-white/90 p-2 shadow-[0_12px_40px_rgba(59,130,246,0.14)] backdrop-blur-sm focus-within:ring-4 focus-within:ring-blue-500/10">
              ...
              <input 
                ...
                className="min-w-0 flex-1 border-0 bg-transparent px-1 py-3 text-sm text-slate-900 outline-none placeholder:text-slate-400 sm:text-base" 
              />
              <button type="submit" className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-indigo-600 text-white transition hover:bg-indigo-700" aria-label="Search">
                <Search size={18} />
              </button>
            </form>
          </div>
