  ...
  return (
    <div className="fixed inset-0 z-30 grid place-items-center bg-slate-950/60 p-4 backdrop-blur-sm" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="detail-title">
        ...
        <div className="px-5 pb-6 sm:px-7"><p className="text-xs font-bold uppercase tracking-widest text-emerald-600">{categoryLabel(listing.category)} · {listing.subcategory || t('localListing')}</p><h2 id="detail-title" className="mt-2 text-2xl font-bold text-slate-900">{listing.title}</h2><p className="mt-3 flex items-center gap-2 text-sm text-slate-500"><MapPin size={16} /> {listing.location}</p><p className="mt-5 whitespace-pre-wrap text-sm leading-7 text-slate-600">{listing.description || t('defaultDescription')}</p><p className="mt-4 text-xs text-slate-400">{t('posted')} {listing.postedAt || t('recently')}</p><a href={`https://wa.me/${whatsappNumber}?text=${message}`} className="mt-6 flex items-center justify-center gap-2 rounded-xl bg-[#25D366] py-4 text-base font-bold text-white hover:bg-[#1db954]" target="_blank" rel="noopener noreferrer"><MessageCircle size={21} /> {t('chatWhatsApp')}</a></div>
      </section>
    </div>
  )
}
