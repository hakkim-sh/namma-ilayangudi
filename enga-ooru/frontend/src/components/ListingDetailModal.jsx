...
import { ChevronLeft, ChevronRight, Edit3, MapPin, MessageCircle, Phone, Trash2, X } from 'lucide-react'
import { useLanguage } from '../context/LanguageContext'

function ListingDetailModal({ listing, imagePlaceholder, onClose, canManage, onEdit, onDelete }) {
    ...
    <div className="fixed inset-0 z-30 grid place-items-center bg-slate-950/60 p-4 backdrop-blur-sm" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="detail-title">
        <div className="relative">
          <div className="flex min-h-64 max-h-[70vh] w-full items-center justify-center overflow-hidden bg-gray-100 sm:min-h-80 dark:bg-gray-800">
            <img src={images[activeImage]} alt={listing.title} className="max-h-[70vh] max-w-full rounded-lg object-contain" />
          </div>
